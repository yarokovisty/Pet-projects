...
import org.pet.project.rickandmorty.feature.character.impl.presentation.state.error
...
internal class CharacterItemViewModel(
    ...
    override fun onIntent(intent: CharacterItemIntent) {
        when(intent) {
            ...
            is CharacterItemIntent.OpenAllEpisodes -> navigateToCharacterEpisodeScreen()
        }
    }
    ...
    private fun navigateToCharacterEpisodeScreen() {
        val character = stateValue.character ?: return

        launchInScope {
            setEvent(CharacterItemEvent.OpenCharacterEpisodeScreen(character))
        }
    }
